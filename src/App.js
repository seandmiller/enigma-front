...
const RotorButton = ({ icon, onClick, disabled }) => (
  <button onClick={onClick} disabled={disabled} aria-label={`Select ${icon} rotor`}>
    <FontAwesomeIcon icon={icon} />
  </button>
);

const PlugBoardButton = ({ letter, onClick }) => (
  <button onClick={onClick} className='plug' aria-label={`Select ${letter} for plugboard`}>
    {letter}
  </button>
);
...
const App = () => {
  ...
  const { plugBoard, letters, enablePlug, handlePlugBoard, setEnablePlug } = usePlugBoard();
  ...
  if (camera) {
    return <Camera handleCamera={() => setCamera(!camera)} encrypt={encryptMsg} plugBoard={plugBoard} rotors={rotors} encryptedWord={encryption} />;
  }
      ...
      <div className='page-wrapper'>
        <div className='top-part'>
          <div className='nested'>
            ...
            <input type='text' name='word' value={word} onChange={handleChange} aria-label="Input word" />

            <h1 className='encrypt-wrapper'>
              {encryption ? (
                <div className='encrypt'>
                  <DigitalEnigma word={scrambled} rotorChoices={rotors} plugBoard={plugBoard} />
                </div>
              ) : (
            ...
            {rotors.length === 3 && (plugBoard.length === 20 || !enablePlug) && (
              <div className='scramble-action-wrapper'>
                <button className='scramble' onClick={() => encryptMsg(word)}>Scramble</button>
                <div className='cam-btn-wrapper'>
                  <div className='camera-btn' onClick={() => setCamera(!camera)} aria-label="Open Camera">
          ...
          <div className='rotors'>
            {rotors.length < 3 && (
              <div className='choices'>
                <RotorButton icon='key' onClick={() => handleRotor('key', 0)} disabled={!rotorSettings[0]} />
                <RotorButton icon='atom' onClick={() => handleRotor('atom', 1)} disabled={!rotorSettings[1]} />
                <RotorButton icon='cogs' onClick={() => handleRotor('cogs', 2)} disabled={!rotorSettings[2]} />
                <RotorButton icon='code' onClick={() => handleRotor('code', 3)} disabled={!rotorSettings[3]} />
                <RotorButton icon='envelope' onClick={() => handleRotor('envelope', 4)} disabled={!rotorSettings[4]} />
              </div>
            )}
            ...
            <div>
              {enablePlug ? (
                <div className='plug-board'>
                  {letters.map(letter => (
                    <PlugBoardButton key={letter} letter={letter} onClick={() => handlePlugBoard(letter)} />
                  ))}
                </div>
              ) : (
